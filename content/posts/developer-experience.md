...
As an industry we should absolutely innovate and build new frameworks, libraries and platform features. We need innovation to push the web forward and find better ways to build and create competition, but we need to be aware of the dynamics at play and have clear strategies to get our frameworks, libraries or platform features into the LLM training corpus and the system prompts of the tools that developers are using and into their minds.

If the industry continues with the focus on maintainability and developer experience, then we will end up in a world where the web is built by LLMs using React and a handful of libraries that are entrenched in the training data. If LLM usage continues to grow at the current rate, then my optimistic take is that we _should_ see a world where each of the tooling vendors has to compete with each other on this homoginized ecosystem and therefore they will need to focus on the quality of the output they produce and they will evolve the output to be more optimized for user experience and not developer experience.

If the industry goes as I believe [then in the long run, the framework will become irrelevant](https://paul.kinlan.me/will-we-care-about-frameworks-in-the-future/) as the models and tools improve to the point people [manipulate the site by words alone](https://blog.almaer.com/english-will-become-the-most-popular-development-language-in-6-years/) and the delivery technology is just an optimized compiled output that meets the needs of the user.

For either scenario to be true we need to start competing on user outcomes. I really want to see Evals and Benchmarks that focus on quality outcomes like Core Web Vitals did for performance. As users of tools that create sites and the tools are competing to attract more users, the ones that output meaningfully better experiences instead of the tools that offer better developer experience, care more about the experience of the site, then the ecosystem of tools and libraries will be incentivized to output code that meets those needs.

As for the raw, naked, web platform? To progress we should be looking to fundamentally new capabilities in the browser—the things that can't be built in user-space, or where there is clear user-experience benefit that can't be achieved with libraries.
