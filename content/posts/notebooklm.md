...
One thing I care deeply about is staying true to the web. FolioLM is designed to enhance your relationship with web content, not replace it. Every source keeps its original URL with an external link icon that opens the original page in a new tab. When the AI cites a source, those citations are clickable &mdash; they open the source URL with [text fragment highlighting](https://developer.chrome.com/docs/web-platform/text-fragments), so you land directly on the relevant passage.
...
This connects to a broader question I've been wrestling with. In [super-apps](/super-apps/) I wrote about how LLMs like Gemini and ChatGPT are becoming the everything-app &mdash; you rarely need to leave them. If that's where things are heading, what happens to the web? In [interception](/interception/) I experimented with having LLMs intercept and transform every web request. In [embedding](/embedding/) I explored how the web needs better primitives for composability if it's going to remain relevant.

FolioLM sits at this intersection. It's not trying to replace the browser or become a closed super-app. It's trying to make the browser itself more capable &mdash; to give you tools to work _with_ web content rather than having that content ingested into some other platform. The web has always been about connecting documents. Links are the fundamental unit of the web's structure. But we've mostly treated that structure as something static &mdash; you follow a link, you read a document, you maybe link to it in your own writing. What if the browser could help you see patterns across documents? Synthesize information? Transform it into new forms?

That's the question I'm exploring with FolioLM and [many other experiments](/projects/).

I should be clear: FolioLM is not a product. It's an experiment. And here's the thing that makes it interesting beyond the features themselves &mdash; I built almost all of it with coding LLMs. Claude, Gemini, OpenAI's tools. Very little handwritten code. This is the intersection I find most exciting right now: LLMs can help you synthesize information across browser tabs in ways we've never been able to do before, _and_ LLMs can help you build the tools to do that synthesis in the first place.
...
That feels like a genuinely new thing. The browser is the runtime. The web is the data source. LLMs are both the capability layer and the means of construction. Anyone can build their own version of this.
