...
On the technical side, it's a Manifest V3 Chrome extension built with TypeScript, Preact, and the Vercel AI SDK. It supports 16+ AI providers including Anthropic, OpenAI, Google Gemini, Groq, Mistral, and Chrome's built-in Gemini Nano (which works offline and costs nothing although can't process anywhere near as much data). Transformations run in the service worker, which means they continue even if you close the side panel.
