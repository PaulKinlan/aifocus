...
-   **Layer 1: read-only access** - `<input type="file" directory>` will let the user select a folder and you can then read the files that are in those folders.
...
Your outer iframe embeds a resource that you control, can be via `srcdoc` that sets a restrictive policy on all network request.

```
<iframe id="jail" sandbox="allow-scripts" srcdoc="
...
-   If we want to stream data into the iframe, we need `sandbox='allow-same-origin'` which gives embedded content a way to access resources that are local to the current origin (cookies etc)
    

There are probably more things that are needed and I don't know if it will be easier to have something like a new dedicated element like `<sandbox href>` that is more suited to running untrusted content. Chrome did [propose](https://wicg.github.io/fenced-frame/) `<fencedframe>` for the Privacy Sandbox project that solves some of these issues. It allows for passing of data between frames in a controlled way and can entirely disable network access with `disableUntrustedNetwork()` - this doesn't work in Safari though so your options are limited.
...
LLMs are incredibly powerful when they have access to Tools. Many of these tools are now provided as MCP servers, which is out of scope for this post at least. The tools that I am talking about are developer provided functions that the LLM can determine which is best to call.

It turns out we have two runtimes in the browser that allow us to run code. The JS environment that we all know and love in the browser, and also the WASM runtime. Both are designed to be able to run code from any untrusted source safely on the user's computer. WASM in particular is incredibly interesting because it enables us to bring decades of software from other systems and run them safely inside the browser - for example, it's possible to create and run a [sqlite database entirely inside the browser](https://sqlite.org/wasm/doc/trunk/index.md) by compiling it to WASM.

Running untrusted code directly in the DOM, or even running code we trust with untrusted input is incredibly risky, so the execution needs to be further sandboxed as far away as possible from our UI environment. We could maybe think about running them in an iframe as noted above, however `WebWorkers` let us isolate our code from the UI in both the DOM manipulation sense and also in the off-the-main-thread sense meaning that we can run code. WebWorkers can also inherit a very strict set of CSP constraints that block network access for example.

Ok, so how feasible is this to put into practice? Well, I built a demo..... Introducing [co-do.xyz](http://co-do.xyz) - a **_demo_** and an **_experiment_** (with no warranties) of everything that we've talked about above.
...
Is it perfect? No. But I think it demonstrates that the browser's 30-year-old security model, built for running hostile code from strangers the moment you click a link, might be better suited for agentic AI than we give it credit for