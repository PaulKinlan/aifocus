...
The gap is not even, either. There is no single "LLM view of the web"; there are many frozen versions of it. One model might be five Chrome releases behind, another seventeen, another twenty-two. Even models from the same vendor can have very different cutoffs. That means a coding tool that swaps models for cost, speed, routing, or task type is also silently swapping the version of the web platform it believes in. The model picker is, in a very real sense, a platform-knowledge picker.
