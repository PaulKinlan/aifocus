...
My first hunch was the deflating one: URLs are not magic context. The averaged numbers backed it up. Adding a bare opaque URL to a prompt did almost nothing on average, and plenty of opaque URLs recovered nothing at all.

But the average was hiding the real story, and the numbers never quite sat right. Item by item, a lot of URLs clearly did pull their content out of nothing but the bare string. Some sites recalled their content, some didn't, and a flat "no influence" couldn't explain the split. So instead of trusting the average I started pulling on the cases that worked, and that is where it got interesting.
...
It wasn't until I pulled the Common Crawl data that it actually clicked. My first thought had been that ChromeStatus simply wasn't crawled. It is: the pages show up in Common Crawl about as often as the arXiv papers that decode perfectly, so on paper they were there. But when I pulled the actual crawled bytes, there was nothing in them. ChromeStatus is a JavaScript app, and the crawler captured an empty shell. The saved page for CSS Gap Decorations is about 3KB of HTML with 22 characters of visible text, "Chrome Platform Status", and not one word about the feature ([here is the actual Common Crawl capture](https://paulkinlan.github.io/url-influence/results/cc-samples/chromestatus-css-gap-decorations.cc.html.txt)). I checked four features and they were all identical empty shells. The arXiv page, by contrast, is server-rendered, so the crawl holds the full title and abstract ([its capture](https://paulkinlan.github.io/url-influence/results/cc-samples/arxiv-attention.cc.html.txt)). So "present in Common Crawl" only ever meant the URL was captured, not that the content was. For these pages the content was never in the crawl at all (this assumes Common Crawl is one of the sources the models trained on, which I think is a safe bet). No content in, no association between the number and the feature, nothing to recall. You can watch every model flatline on the bare ChromeStatus id, then recover the feature once handed the actual page, [in the per-test view here](https://paulkinlan.github.io/url-influence/results/dashboard.html#test=css-gap-decorations).

To kill the "maybe it just wasn't crawled" doubt entirely, I tried a case where the content is beyond question in the model. Every Wikipedia article has an internal numeric id you can address directly: `en.wikipedia.org/?curid=24544` is Photosynthesis. The article is server-rendered, it is in the crawl, and the models obviously know photosynthesis cold. From the curid alone, they recover nothing. Zero, every time, across all five articles I tried (Photosynthesis, Mitochondrion, HTTP 404, Bitcoin, and the Transformer). Ask by name and they score perfectly, paste the article and they score perfectly, give the bare numeric id and it is a flat 0.00. You can see every model on it [in the per-test view here](https://paulkinlan.github.io/url-influence/results/dashboard.html#test=wiki-curid-photosynthesis), and the same for [the Transformer article](https://paulkinlan.github.io/url-influence/results/dashboard.html#test=wiki-curid-transformer-dl). That is the whole finding in one test: the content being in the model is necessary but not enough. The id has to be something people actually write down and cite, and a curid never is.
...
So I tried to measure it. I streamed a sample of Common Crawl, kept the real (200, text/html) pages, and counted the ones with almost no readable text that also carried a client-render signature. To be careful about it, "no readable text" means no visible text AND no content hiding in inline JSON: a Next.js page often ships almost nothing visible but embeds the whole page as JSON in a script tag, and a model training on the raw bytes does see that, so those count as content-present, not shells. Across about 568,000 pages from 48 crawl files, 1.2% were confident shells. It is a sample and the detection is conservative (a shell still has to announce a framework, jQuery-on-load, or a bare app skeleton), so read it as a floor, but "more than one in a hundred crawled pages is effectively blank to the model" is a striking place to land. These are not small files either: the shells average about 53KB of HTML, all bundles and markup, with next to no text. The biggest bucket was pages with no attributable framework at all, then jQuery-on-load pages, then Next.js. For context, jQuery shows up on two thirds of all crawled pages and Bootstrap on a fifth, but those are mostly fine, sitting on server-rendered HTML; it is specifically the client-rendered ones that vanish.

The part I did not expect: it gets worse the more popular the site is. Joining each page to a domain ranking, the shell rate climbs from 0.85% across the long tail (unranked domains) to 1.9% in the 10k-100k band and 2.5 to 2.8% among the top 10,000 sites. The polished, well-funded, modern front of the web is the part most likely to be a blank shell in the crawl. So this is not a problem of obscure sites being neglected; it is the opposite. (Every number here is from a sample I have published, including the exact list of crawl files used, so it can be re-run and checked.)

{{< iframe "https://paulkinlan.github.io/url-influence/results/shell-survey.html" 760>}}

In a world where people increasingly ask a model instead of opening ten tabs, being absent from the training data is a real discoverability problem, and I don't think it is well understood yet. There is a lot more to dig into: does the rate differ between providers, given some of them run search crawlers that do execute JavaScript? Does it correlate with how popular a site is? I would be glad if this nudged someone to take it further.
