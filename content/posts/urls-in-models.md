...
So I tried to measure it. I streamed a sample of Common Crawl WARC files, kept the real (200, text/html) pages, and counted the ones with almost no visible text that also carried a client-render signature (a framework marker, a jQuery handler that builds the page on load, or a bare SPA skeleton). Across about 74,000 pages, roughly 1.9% were confident shells. It is a sample and the detection is conservative, so treat it as a floor rather than a precise figure, but "around one in fifty crawled pages is effectively blank to the model" is a striking place to land. The biggest single bucket was pages with no attributable framework at all (just an empty skeleton), then jQuery-on-load pages, then Next.js. For context, jQuery turns up on about two thirds of all crawled pages and Bootstrap on a fifth, but those are mostly fine: they sit on server-rendered HTML. It is specifically the client-rendered ones that vanish.
