...
I thought it might be interesting to look at some of the existing permission models, because each of them gets part of this story right.
...
What all of these share is that they host _attempts_ to enforce the boundary, and the program declares what it needs upfront. None prompt the user for individual operations the way an OS file picker or your chat client does. None let the program escalate beyond what the manifest, the flags, or the bindings declared. That's the shape that's missing for prompt-programs running in random Node processes today, and it's why I keep coming back to the conclusion that a useful agent is more likely to look like a Chrome Extension than a normal web app.
...
While I feel comfortable running this type of system, the note above "given the correct security and privacy guardrails" does a huuuuggeee amount of heavy lifting and I wouldn't recommend this for all but the most technical user right now. It's pretty easy to see from the examples I have here that prompt injection could be probably relatively easily achieved given the existing known issues with how LLMs can easily be manipulated based on input that comes from untrusted sources... (side note, it feels like we need a `taint` mode like we have in HTMLCanvas)
